...
class DelProject(webapp2.RequestHandler):
    def get(self):
        ...
        if session["pos"]==1 or session["pos"]==4:
            ...
            for eop in emplonp:
                eop.empl.__setattr__('assigned',False)
                eop.empl.save()
                eop.delete()
            proj.delete()
            self.redirect('/projects')
...
class EditProject(webapp2.RequestHandler):
    def post(self):
        ...
        if session["pos"]==1 or session["pos"]==4:
            ...
            id=int(raw_id)
            project = Project.get_by_id(id)
            project.__setattr__('name',self.request.get('projName'))
            project.__setattr__('type',self.request.get('projType'))
            project.__setattr__('description',self.request.get('projDescription'))
            project.__setattr__('pmid',int(self.request.get('projPmId')))
            ...
            project.__setattr__('startdate',start_date)
            project.__setattr__('enddate',end_date)
...
class EmployeeAloc(webapp2.RequestHandler):
    def get(self):
        ...
        if session["pos"]==1 or session["pos"]==4:
            ...
            emplon.put()
            employee.__setattr__('assigned',True)
            employee.save()
...
class EmployeeDealoc(webapp2.RequestHandler):
    def get(self):
        ...
        if session["pos"]==1 or session["pos"]==4:
            employee = Employee.get_by_id(int(self.request.get('emplid')))
            employee.__setattr__('assigned',False)
            employee.save()
...
class RemoveEmployeeSkill(webapp2.RequestHandler):
    def get(self):
        ...
        if viewer.key().id == employee.key().id:
            ...
            skillempl = employee.employeeskills_set.filter('skill = ',skill.key()).get()
            skillempl.delete()
...
class EditEmployeePage(BaseHandler):
    def get(self):
        ...
        if session["pos"]==2 or session["pos"]==4:
            employee = Employee.get_by_id(int(self.request.get('emplid')))
            vals = {'employee':employee,
                    'position':employee.__getattribute__('position'),
                    'url':session["url"]}
...
class EditEmployee(webapp2.RequestHandler):
    def post(self):
        ...
        if session["pos"]==2 or session["pos"]==4:
            employee = Employee.get_by_id(int(self.request.get('emplId')))
            employee.__setattr__('name',self.request.get('emplName'))
            employee.__setattr__('user',User(self.request.get('emplUser')))
            employee.__setattr__('position',self.request.get('emplPosition'))
            employee.__setattr__('years',self.request.get('emplExp'))
